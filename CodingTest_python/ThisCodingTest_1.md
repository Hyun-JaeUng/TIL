...
* 사전 자료형은 키(Key)와 값(Value)의 쌍을 데이터로 가지는 자료형
  * 앞서 다룬 리스트나 튜플이 값을 순차적으로 저장하는 것과는 다름
* 사전 자료형은 키와 값의 쌍을 데이터로 가지며, 원하는 **변경 불가능한(immutable) 자료형을 키로 사용할 수 있음**
...
* ```python
  ...
  # key: 사과, 바나나, 코코넛 // value: Apple, Banana, Coconut
  
  print(data)
  
  # 실행결과: {'사과': 'Apple', '바나나': 'Banana', '코코넛': 'Coconut'}
  ```
...
* ```python
  ...
  # 실행결과: 사과를 키로 가지는 데이터가 존재합니다.
  ```

* 사전자료형에서는 키와 값을 별도로 뽑아내기 위한 함수를 지원함
...
* 혹은 중괄호`{}` 안에 각 콤마를 기준으로 구분하여 삽입함으로써 초기화 할 수 있음
