...
* try - except - else - finally 를 이용하여 예외 처리할 수 있음
* 간단히 `try:  except:` 까지만 사용하는 것도 가능
  * 예외가 발생하지 않으면 except 블록 무시하고 다음 문장 실행
  * 상황 자체를 해결하지는 않으나 예외를 인식/처리 하는 기회 제공 

#### raise 명령

* 고의적으로 예외를 발생시킴
* 작업을 위한 선결조건이 맞지 않거나 문제 발생할 경우 호출원으로 사용
  * `raise ValueError`
...
* 속성과 동작을 하나의 범주로 묶어서 실세계의 사물을 프로그램 코드로 흉내냄
* 객체 지향의 가장 기본적 개념
...
* 인스턴스라는 말은 특정 객체가 어떤 클래스의 객체인지를 관계 위주로 설명할 때 사용하는 말로 객체와 거의 동일한 개념이라고 생각하면 됨 

#### 생성자

* 클래스 선언 형식
* `__init 생성자__` : 통상 객체 초기화
...
* 인스턴스 변수: 객체마다 만들어지는 변수

  * `객체.인스턴스변수` 로 사용

* 클래스 변수: 클래스에 속한 변수

  * 클래스 변수에 대한 접근: `클래스명.클래스변수`, `객체.클래스변수` 두가지 다 됨
  ...
  * 모든 객체에 공통적인 변수값을 가진다면 클래스 변수를 이용하는 것이 효율적!

## Day 12 (0120)

> 예제 코드 및 교안 (패키지), 파이썬 공부 여기까지!  (강의교안 7, 45p까지 완료)
>

* 관례적으로 클래스 첫글자는 대문자, 함수 첫글자는 소문자로 정하기
* `import 다른파이썬모듈` 하면 해당 파이썬 파일에 있는 함수와 코드가 모두 실행됨
  * 이를 방지하기 위해 `if __name__ == "__main__"` 을 사용하여 import 시 불필요한 코드가 실행되지 않게 함
  * 주인공으로 수행되는 모듈의 경우 모듈명이 수행하는 동안에만 `__main__` 으로 바뀐다.

### 모듈

* 파이썬 코드를 저장하는 기본 단위
* 타 모듈에 함수 제공하는 모듈에서는 테스트 코드를 조건문으로 감쌈 (위에서 말한 `if __name__`)
* 모듈은 임포트하는 파일과 같은 디렉토리에 있어야 함
  * 특정 폴더에 두려면 임포트 패스에 추가해야함

### 패키지

* 모듈을 담는 디렉토리를 패키지라 한다.
* 단일 모듈에 비해 호출문 길어지는 불편함은 있음
  * 다만 함수명이나 모듈명에 충돌 발생하지 않음
* `__init__.py`: 해당 디렉토리(패키지)의 모든 모듈을 한꺼번에 불러올 때 사용
  * 대신 디렉토리에 해당 py 있어야 하고, 파일 안에서 임포트할 대상 모듈 리스트를 명시해야함
  * 조건이 충족된다면 `import mypack.calc import *` 으로 호출 가능
* 서드 파티 모듈을 이용할 때 dir 내장 함수를 사용하여 모듈에 있는 함수나 변수 목록을 볼 수 있음

#### 추후 해야할 것

* get함수 관련 내용 조사 후 추가 
* markdown 파일 깃 업로드 시 공백 안되는 문제 해결방법 찾기
* Day10, Day11 실습과 강의교안은 복습 완료했지만 Test 예제 코드는 복습 필요
* 메서드 강의교안 내용 질문, class13,  - 영민형
